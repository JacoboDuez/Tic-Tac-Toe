...
const Square = ({ takeTurn, id }) => {
  ...
  return (
    <button
      ...
      onClick={() => {
        // if the box has not been selected then I proceed with the next actions
        if (!checkedOrNot(statusPerSquare,id)){
        setTik(takeTurn(id));
        setFilled(true);
        setStatusperSquare(...statusPerSquare,length(id));
...
const checkedOrNot = (statusPerSquare,id)=>{
  let selected = false;
  for(let i=1;i<statusPerSquare.length;i++){
    if(id==i){
      if(statusPerSquare[i]==true){
        selected = true;
        break;
      }
      else{
        statusPerSquare[i] = true;
        break;
      }
    }
  }
